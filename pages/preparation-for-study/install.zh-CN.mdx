...
* 安装到本地：将 ComfyUI 安装在自己的电脑中，这样你可以在本地运行 ComfyUI。这种方式基本是 0 成本。但如果你的电脑 GPU 配置相对较差，有可能生图的速度会比较慢（我个人更推荐这个方式）。
...
<Tabs items={['VS Code 方法（推荐）', 'Terminal 方法']}>
  ...
  <Tabs.Tab>
    ...
    <Steps>
      ...
      ### cd 到项目文件夹
      打开 Terminal 输入 cd，然后空格，接着黏贴上一步复制好的代码。然后需要注意，此时你还需要在最后补上 ComfyUI 然后按下回车，如果顺利的话，会是这样：
      <PhotoProvider>
