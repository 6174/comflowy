...
- Text Encoder：在这个 Workflow 上带有两个输入框的组件，叫 CLIP Text Encode。还记得上一章我们提到过 Stable Diffusion 的 Text Encoder 用的是 CLIP 里的 Text Encoder 吗？其实这个组件和上一章提到的 Text Encoder 是一个东西。

- Image Information Creator：就是图里的 KSmapler，名字差异很大。后续我会详细讲解。还有就是 Creator 里的 Random Image，就是 Workflow 上的 Empty Latent Image，上一章我们也提到，整个生图过程是在 Latent Space 里进行，所以在 ComfyUI 里的这个节点叫 Empty Latent Image。

- Image Decoder：就是上一章提到的 VAE Decoder 了。
