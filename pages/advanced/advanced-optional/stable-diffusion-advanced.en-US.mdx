...
<Steps>
  ### Replace KSampler with KSampler XL
  ...
  First, double-click the blank space, search for Text Box, and add it to the workflow. Then right-click on one of the CLIP Text Encode nodes, select Convert text to widget, and the input box of the CLIP Text Encode node will disappear, and an extra Text input will be added to the input end. At this point, connect this Text input to the Text Box we just added. 

  You need to repeat the above steps. The result should look like this:

  Then add two more CLIP Text Encode nodes, connect them to the second KSampler, and connect them to the previously added Text Box. The final situation should look similar to this:
