...
* 除了以上用法外，还有一种就是仅将原图的风格输入给模型，这样生成的图片就只会有原图的风格，而不会有原图的内容。这种 workflow 一般称为 Style model workflow。
...
另一种重绘，则是只用图片的风格，而不用图片的内容。其实实现方法跟 unCLIP model workflow 很类似，只是需要将图片的内容去掉，只保留风格。
