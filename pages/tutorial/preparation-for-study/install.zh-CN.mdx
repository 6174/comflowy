...
<br/>
<PhotoProvider>
  <PhotoView src="/install-comfyui-and-model/install-comfyui-hero.png">
      <img src="/install-comfyui-and-model/install-comfyui-hero.png" alt="" width="100%" style={{ display: "block", margin: "0 auto" }} className="rounded-lg"/>
  </PhotoView>
</PhotoProvider>
...
最后一种方法是简单的，你只需要注册一个账号，然后就可以使用 [Comflowy](https://app.comflowy.com) 的 ComfyUI 云端版本。无需处理任何的安装问题。
...
需要注意，你最终安装的界面可能与我稍微有点不同，这有可能是我安装了插件导致的，也有可能是 ComfyUI 更新导致的。你只要能看到各种节点和连线就是正常的了：
...
**Q2: 终端显示 ERROR: Could not find a version that satisfies the requirement pyyaml (from versions: none) 该如何处理？**
