...
<Steps>
  ### 加载 Default workflow
  按照惯例，我们先从默认的 workflow 开始。点击右侧面板里的 Load Default 按钮，加载完

  ### 添加 VAE Encode(for Inpainting)
  双击空白处，输入 Inpainting，添加该节点。你会看到这个节点上有个 grow_mask_by 的配置项，我这一般会设置成 6～8。
  ...
  除了要调整模型外，还需要将 KSampler 里的 denosie 调成 0.85，当然你也可以根据自己的测试来，这个值小于 1 即可。
